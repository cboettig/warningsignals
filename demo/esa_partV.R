...
#fn_mc <- montecarlotest(fn$const, fn$test, cpu=16, nboot=200)
...
#tp_mc <- montecarlotest(tp$const, tp$test, cpu=16, nboot=200)
#tp_taus <- tau_dist_montecarlo(ibm_critical, tp$const, tp$timedep, nboot=500, cpu=16)
#
#load("small_tau.Rdat") # 
#tn <- fit_models(ibm_critical, "LSN")
#tn_mc <- montecarlotest(tn$const, tn$test, cpu=16, nboot=200)
...
#fp_mc <- montecarlotest(fp$const, fp$test, cpu=16, nboot=200)
...
load("partV.Rdat")
taus <- reformat_tau_dists(list(fn=fn_taus, tp=tp_taus, fp=fp_taus, tn= tn_taus))

load("false_negative.Rdat")
plot_indicator(ibm_critical, "CV")
roc_curve(taus[[1]], main = )
